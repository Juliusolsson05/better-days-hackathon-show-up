...
export function parseTrackingRequest(input: unknown): {
  ...
  if ('events' in body) {
    ...
    incoming = [{ name: body.name, group_id: body.group_id, props: body.props }];
  } else {
  ...
  if (incoming.length > MAX_BATCH) {
    throw new TrackingRequestError(`request may contain at most ${MAX_BATCH} events`);
  }
  ...
  return {
    ...
    rejected: parsed.filter((item) => !ALLOWED.has(item.name)).map((item) => item.name),
  };
}

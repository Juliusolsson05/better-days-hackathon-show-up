...
import { parseTrackingRequest } from './tracking.ts';
...
Deno.test('malformed tracking payloads fail before membership or ClickHouse work', () => {
  ...
  assertThrows(() => parseTrackingRequest(null), Error, 'body must be a JSON object');
  assertThrows(() => parseTrackingRequest({ events: 'rsvp' }), Error, 'events must be an array');
  assertThrows(() => parseTrackingRequest({ events: [null] }), Error, 'event must be a JSON object');

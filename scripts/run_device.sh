...
# Keep the shell check even though main.dart validates in release: this message preserves the
# actionable variable name before a several-minute iOS build starts. Fixture review uses the
# structurally separate lib/main_reference.dart entrypoint and cannot be selected here.
: "${SUPABASE_URL:?run_device.sh needs SUPABASE_URL}"
: "${SUPABASE_ANON_KEY:?run_device.sh needs SUPABASE_ANON_KEY}"
